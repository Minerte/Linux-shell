...
EOF
    # Prep for boot disk
    mkdir -p /media/extern-usb
    mount "${sel_disk_boot}2" /media/extern-usb
    ...
    gpg --decrypt crypt_key.luks.gpg | cryptsetup luksFormat --key-size 512 --cipher aes-xts-plain64 "${sel_disk}2" - gpg: AES256.CFB encrypted data || { echo "Failed  to decrypt keyfil and encrypt diskt"; exit 1; }
